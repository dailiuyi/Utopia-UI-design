...
本文件是可复用的实现约束，不是已编译的 Unity 组件包。纵横屏决斗与生命值均为原始模板；适配已有工程时以实际朝向、玩法、输入和支持平台为准。官方链接于 2026-09-22 核对；包文档举例不等于要求安装该版本。
...
项目需要旋转时，准备与战斗使用分别编排的纵横布局，共享数据与场景对象；始终竖屏的工程保留实际方向。可采用不同页面根节点/Prefab/模板，不把竖屏界面整体横向拉伸。允许横屏左/右时分别检查刘海与手势区；是否锁某一侧由产品决定。
...
可复用资产优先为少量分层面板、切角/角框、状态刻度、机械细线、图标及中文字体（警示纹理只用于警告）。使用 9-slice 时检查切角是否被拉伸；使用程序绘制时检查多分辨率边线。不要为每个控件导出一整张大背景。随包海报只作设计参考，不是待导入客户端的 UI 贴图。
...
完整路径：首次进入 → 相机授权 → 定位 → 场地绑定 → 按项目要求切换朝向（如需） → 游戏 → 位置失效/恢复 → 结算 → 返回准备。再验证权限拒绝、定位失败、场地绑定失败、失焦恢复和旋转期间的输入取消。操作未定时只验收共享 HUD 与接口，完整可玩对战仍待定。
